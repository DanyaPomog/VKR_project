...
inset_full_polutions()


#поиск машины по названию
def read_singl_str(id):
    try:
        print("начало чтения одиночной строки")
        select_one = """SELECT * FROM polutions WHERE id = ?"""

        c.execute(select_one, (id, ))
